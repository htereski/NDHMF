...
onMounted(() => {
  ...
  Echo.private(`new-chat-message.${props.chat.id}`)
...
<template>
  <div class="fixed bottom-0 right-0 w-full md:w-1/3 lg:w-1/4 bg-white shadow-lg border border-gray-300 rounded-t-lg">
    <div class="bg-blue-500 text-white p-3 flex justify-between items-center cursor-pointer" @click="toggleChat">
      <span>Chat</span>
      <span class="text-lg font-bold">{{ isChatOpen ? "−" : "+" }}</span>
    </div>

    <div v-if="isChatOpen" ref="messagesContainer"
